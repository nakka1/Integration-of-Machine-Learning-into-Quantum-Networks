...
# Models that require NO training and are entirely deterministic given
# (X_train, y_train, X_test, y_test): evaluating them across multiple
# seeds would just re-run the identical, expensive quantum-simulation
# loop several times for zero additional information, so
# `run_model_comparison` evaluates each of these on a single nominal
# seed rather than looping over the full `comparison_cfg.seeds` list.
NAIVE_OR_ORACLE_MODELS = {"Persistence", "MovingAverage", "Oracle"}

# Non-deployable reference points (see `baselines.OraclePredictor`'s
# docstring): excluded from the ranked decision matrix and its
# sensitivity analysis (comparing real, deployable candidates against a
# perfect-information oracle would distort the min-max normalization of
# every other row), but still reported in `results_df` and available for
# `statistics_tests.compare_models_statistically`.
DECISION_MATRIX_EXCLUDED_MODELS = {"Oracle"}
...
def _build_predictor(name: str, seed: int, device: torch.device,
    ...
    """
    Trains (or, for the naive/oracle reference baselines, simply
    constructs) one predictor of `name` and returns an object compatible
    with `DigitalTwinOrchestrator` (a trained `nn.Module`, a
    `_SklearnRegressorAdapter`, or a `_NaiveSequentialPredictorBase`
    subclass).

    `X_test`/`y_test` are only required for `"Persistence"`,
    `"MovingAverage"`, and `"Oracle"` (they are built directly over the
    test-set true-fidelity sequence, not trained on `X_train`/`y_train`
    at all -- see `baselines.build_persistence_baseline` and friends).
    ...
    if name == "Persistence":
        return build_persistence_baseline(y_train, y_test)
    if name == "MovingAverage":
        return build_moving_average_baseline(y_train, y_test, window=baseline_cfg.moving_average_window)
    if name == "Oracle":
        return build_oracle_baseline(y_test)
...
def run_model_comparison(X_train: torch.Tensor, y_train: torch.Tensor,
                          ...
                          ) -> Tuple[pd.DataFrame, dict, pd.DataFrame, dict, Tuple[pd.DataFrame, pd.DataFrame, str]]:
    """
    ...
    model_names : list[str], optional
        Subset/order of predictors to include. Default: all eight
        (`["EdgeLSTM+CS-MSE", "LSTM+MSE", "RandomForest", "XGBoost",
        "Transformer", "Persistence", "MovingAverage", "Oracle"]`).
        "XGBoost" is silently skipped (with a printed warning) if
        `xgboost` isn't installed or `comparison_cfg.include_xgboost` is
        `False`. "Persistence"/"MovingAverage"/"Oracle" are deterministic
        reference baselines (see `baselines.py`): each is evaluated on a
        single nominal seed rather than the full `comparison_cfg.seeds`
        list (looping would be wasted, identical-result compute), and
        "Oracle" -- a non-causal, perfect-information upper bound, not a
        deployable model -- is excluded from `decision_matrix_df` and the
        sensitivity analysis (see `DECISION_MATRIX_EXCLUDED_MODELS`)
        while still appearing in `results_df`.
    ...
    model_names = model_names or ["EdgeLSTM+CS-MSE", "LSTM+MSE", "RandomForest", "XGBoost", "Transformer",
                                    "Persistence", "MovingAverage", "Oracle"]
    ...
    for name in model_names:
        ...
        # Deterministic reference baselines don't benefit from multi-seed
        # averaging (every seed produces the identical result) -- evaluate
        # them once, on the first configured seed, instead of repeating an
        # expensive, identical quantum-simulation pass `len(seeds)` times.
        effective_seeds = [comparison_cfg.seeds[0]] if name in NAIVE_OR_ORACLE_MODELS else comparison_cfg.seeds

        print(f"[{name}] {'evaluating (deterministic, single pass)' if name in NAIVE_OR_ORACLE_MODELS else f'training on {len(effective_seeds)} seeds'} ...")
        seed_runs = []
        skipped = False
    ...
    } for r in rows if r["Model"] not in DECISION_MATRIX_EXCLUDED_MODELS]
