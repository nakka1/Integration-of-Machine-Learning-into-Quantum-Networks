...
`train_edge_lstm` explicitly accepts a `seed` and calls `seed_everything`
(Python `random` + NumPy + Torch CPU/CUDA generators) before any
parameter initialization -- a prerequisite for the multi-seed averaging
implemented in `pareto_sweep.py` (each seed must produce a genuinely
different, reproducible weight initialization, on CPU and GPU alike).
"""
...
class EdgeLSTM(nn.Module):
    ...
    def __init__(self, input_size: int = 2, hidden_size: int = 16, num_layers: int = 1) -> None:
        super().__init__()
        self.lstm = nn.LSTM(
...
class CS_MSELoss(nn.Module):
    ...
    def __init__(self, threshold: float = 0.65, lambda_penalty: float = 10.0,
                 ...
                 max_discard_rate: float = 0.60) -> None:
        super().__init__()
...
def train_edge_lstm(model: nn.Module, X_train: torch.Tensor, y_train: torch.Tensor,
                     ...
                     epochs: int = 120, lr: float = 3e-3, device: torch.device | None = None,
    ...
    When provided, `seed` is re-applied via `seed_everything` at the start
    of this function (Python `random` + NumPy + Torch CPU/CUDA generators
    -- see `reproducibility.py`), for reproducibility of the optimization
    loop itself (Adam's internal operation order, etc.). **Important**:
    since training here is full-batch (no DataLoader/shuffling), the only
    real source of variation between seeds is the model's weight
    initialization -- and that initialization must already have happened
    *before* this call, with the same seed applied immediately before
    `EdgeLSTM(...)` is instantiated. This is exactly the pattern (seed ->
    build model -> train) that `pareto_sweep.run_pareto_sweep` follows on
    every round of the multi-seed averaging, ensuring each round starts
    from a genuinely different, reproducible initialization.
    """
    if seed is not None:
